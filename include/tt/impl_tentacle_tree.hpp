...
template <Point3d PointT, typename Allocator>

bool isPointInNode(const std::array<typename Node<PointT, Allocator>::CoordT, 3> &point,

                   const Node<PointT, Allocator> &node) {
...
bool isBoxInNode(const BoundingBox<typename Node<PointT, Allocator>::CoordT> &box, const Node<PointT, Allocator> &node) {

    return isPointInNode<PointT, Allocator>(box.min_coords, node) &&

           isPointInNode<PointT, Allocator>(box.max_coords, node);
...
auto toBoundingBox(const Node<PointT, Allocator> &node) {
...
bool isBoxOverlappingNode(const BoundingBox<typename Node<PointT, Allocator>::CoordT> &a,
...
bool isNodeInBox(const Node<PointT, Allocator> &node, const BoundingBox<typename Node<PointT, Allocator>::CoordT> &box) {
...
bool isSphereInNode(const Node<PointT, Allocator> &node, const PointT &sphere_center,

                    typename Node<PointT, Allocator>::CoordT sphere_radius) {
    ...
    using CoordT = typename Node<PointT, Allocator>::CoordT;
...
bool isNodeInSphere(const Node<PointT, Allocator> &node, const PointT &sphere_center,
...
bool doesNodeOverlapSphere(const Node<PointT, Allocator> &node, const PointT &sphere_center,
...
template <Point3d PointT, typename Allocator, ForwardPoint3dIterator BeginIt, ForwardPoint3dIterator EndIt>

std::array<std::vector<PointT, Allocator>, 8>

distributePointsToOctants(BeginIt begin, EndIt end,

                          const std::array<PointCoordinateTypeT<PointT>, 3> &center) {

    std::array<std::vector<PointT, Allocator>, 8> octant_points;
...
computeParentCenter(const Node<PointT, Allocator> &node,
...
makeParentAndLinkNode(std::unique_ptr<Node<PointT, Allocator>> node,

                      const std::array<PointCoordinateTypeT<PointT>, 3> &target) {

    using CoordT = PointCoordinateTypeT<PointT>;

    auto parent_node = std::make_unique<Node<PointT, Allocator>>();
...
growTreeUntilAbsorption(std::unique_ptr<Node<PointT, Allocator>> node,

                        const BoundingBox<PointCoordinateTypeT<PointT>> &bbox) {

    std::unique_ptr<Node<PointT, Allocator>> root = std::move(node);
...
template <std::random_access_iterator BeginIt, std::random_access_iterator EndIt, Point3d PointT, typename Allocator>

void insert(BeginIt begin, EndIt end, Node<PointT, Allocator> &node, std::size_t bucket_size,
...
void splitAndInsertPoints(BeginIt begin, EndIt end, Node<PointT, Allocator> &node,
    ...
    auto octant_points = impl::distributePointsToOctants<PointT, Allocator>(begin, end, node.center);
    ...
    for (std::size_t octant_idx = 0; octant_idx < 8; ++octant_idx) {
        ...
        if (not node.children[octant_idx]) {
            ...
            node.children[octant_idx] =
                ...
                std::make_unique<Node<PointT, Allocator>>(child_center, child_half_extent);
...
void insertIntoLeaf(BeginIt begin, EndIt end, Node<PointT, Allocator> &node, const std::size_t bucket_size,
...
bool isNodeSmallestPossible(const Node<PointT, Allocator> &node, PointCoordinateTypeT<PointT> min_extent) {
...
bool hasChildren(const Node<PointT, Allocator> &node) {

    return std::any_of(node.children.begin(), node.children.end(),

                       [](const std::unique_ptr<Node<PointT, Allocator>> &c) { return c != nullptr; });
...
void insert(BeginIt begin, EndIt end, Node<PointT, Allocator> &node, const std::size_t bucket_size,
    ...
    if (not hasChildren(node)) {
        ...
        if (will_bucket_overflow and not is_smallest_possible) {
            ...
            auto octant_points = impl::distributePointsToOctants<PointT, Allocator>(
...
void collectLeafNodes(std::vector<const Node<PointT, Allocator> *> &leaves, const Node<PointT, Allocator> &node) {
...
std::vector<const Node<PointT, Allocator> *> collectLeafNodes(const Node<PointT, Allocator> &node) {

    std::vector<const Node<PointT, Allocator> *> leaves;
...
std::vector<std::vector<PointT> *> collectLeafPoints(std::vector<const Node<PointT, Allocator> *> &leaves) {
...
std::vector<std::vector<PointT> *> collectLeafPoints(const Node<PointT, Allocator> &node) {
...
auto distance(const Node<PointT, Allocator> &node, const PointT &point) {
...
Node<PointT, Allocator> &findClosestLeafNode(Node<PointT, Allocator> &node, const PointT &point,
...
bool knnSearch(const Node<PointT, Allocator> &node, const PointT &query_point, std::size_t k,
...
PointCoordinateTypeT<PointT> nodeToPointDistance(const Node<PointT, Allocator> &node, const PointT &point) {
...
void radiusSearch(const Node<PointT, Allocator> &node, const PointT &query_point,
...
TentacleTree<PointT, Allocator>::TentacleTree(std::size_t bucket_size, CoordT min_extent, const Allocator &alloc)

    : bucket_size_(bucket_size), min_extent_(min_extent), allocator_(alloc) {}
...
void TentacleTree<PointT, Allocator>::insert(BeginIt begin, EndIt end) {
...
void TentacleTree<PointT, Allocator>::init(BeginIt begin, EndIt end) {
...
std::unique_ptr<Node<PointT, Allocator>> TentacleTree<PointT, Allocator>::createNode(const std::array<CoordT, 3> &center,
    ...
    auto node = std::make_unique<Node<PointT, Allocator>>(center, half_extent);
    ...
    if (keep_splitting) {
        ...
        auto octant_points = impl::distributePointsToOctants<PointT, Allocator>(begin, end, center);
...
void TentacleTree<PointT, Allocator>::boxDelete(const BoundingBox<CoordT> &box) {
...
std::unique_ptr<Node<PointT, Allocator>> TentacleTree<PointT, Allocator>::boxDelete(const BoundingBox<CoordT> &box,
...
TentacleTree<PointT, Allocator>::SearchResult TentacleTree<PointT, Allocator>::knnSearch(const PointT &query_point,
...
TentacleTree<PointT, Allocator>::radiusSearch(const PointT &query_point, CoordT radius) {
